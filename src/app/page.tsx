...
export default function LoginPage() {
  ...
  useEffect(() => {
    if (typeof window !== 'undefined') {
      ...
      if (urlParams.get('denied') === '1') {
        toast('error', 'Akses Ditolak!', 'Anda harus login terlebih dahulu.');
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!res.ok || !data.success) {
        toast('error', 'Login Gagal', data.error || 'Terjadi kesalahan sistem')
        setLoading(false)
        return
      }

      // Login berhasil - simpan juga di localStorage untuk komponen lama
      localStorage.clear()
      ...
      if (data.role === 'it') {
        ...
        router.push(`/sppg/dashboard/${data.unit_id}`)
      }
    } catch (err: any) {
  ...
  return (
    <div className="min-h-screen bg-white flex flex-col md:flex-row font-sans overflow-hidden">
      ...
      <div className="md:w-1/2 flex flex-col items-center justify-center p-8 md:p-20 bg-white relative">
        ...
        >
          ...
          <div className="text-center md:text-left">
            ...
            <p className="text-sm text-slate-500 font-medium">Yok login sekarang!</p>
          </div>
      ...
      <a
        href="https://wa.me/6281330110828?text=Halo%20Mba%20Aisha,%20saya%20butuh%20bantuan%20terkait%20sistem%20SPPG."
        target="_blank"
