  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (error || !user) {
        ...
        // 4. LOGIKA NAVIGASI OTOMATIS BERDASARKAN ROLE
        if (user.role === 'admin') {
          router.push('/it') // Admin langsung ke Dashboard IT
        } else if (user.role === 'korwil') {
          router.push('/korwil') // Korwil ke Monitor Global
        } else if (user.role === 'sppg') {
          // Akun SPPG langsung masuk ke dashboard input miliknya tanpa pilih unit lagi
      ...
      <div className="md:w-1/2 bg-[#0F2650] flex flex-col items-center justify-center p-12 text-center text-white relative">
        ...
        >
          <div className="bg-white/10 backdrop-blur-md p-6 rounded-3xl inline-block border border-white/20 mb-4 shadow-2xl">
            <Image src="/logo.png" alt="Logo Pasuruan" width={140} height={140} className="object-contain" />
          </div>
          <div className="space-y-4">
            <h1 className="text-4xl md:text-5xl font-black tracking-tighter leading-tight">
              Sistem Manajemen <br/> 
              <span className="bg-yellow-400 text-[#0F2650] px-3 py-1 rounded-xl uppercase">Operasional SPPG</span> <br/>
              Kab. Pasuruan
            </h1>
            <p className="text-sm text-indigo-200 font-medium leading-relaxed opacity-80">
              Platform terintegrasi untuk pemantauan distribusi gizi dan operasional SPPG di wilayah Kabupaten Pasuruan.
            </p>
          </div>
        </motion.div>
      ...
      <div className="md:w-1/2 flex flex-col items-center justify-center p-8 md:p-24 bg-white relative">
        ...
        >
          <div className="text-center md:text-left">
             ...
             <p className="text-sm text-slate-400 font-medium tracking-wide">Masuk untuk mengakses layanan operasional SPPG</p>
          </div>

          <form onSubmit={handleLogin} className="space-y-6">
            <div className="space-y-2">
              <label className="text-[11px] font-black text-slate-500 uppercase tracking-widest ml-1">Email / Akun SPPG</label>
              <div className="relative group">
                ...
                <input required type="email" className="w-full pl-12 pr-4 py-4 bg-slate-50 border border-slate-100 rounded-2xl text-sm font-bold text-slate-700 outline-none focus:ring-4 focus:ring-indigo-500/5 focus:border-indigo-500 transition-all" placeholder="sppg@mbg.com" onChange={(e) => setEmail(e.target.value)} />
              </div>
            </div>
            ...
            <div className="space-y-2">
              ...
              <div className="relative group">
                ...
                <input required type="password" className="w-full pl-12 pr-4 py-4 bg-slate-50 border border-slate-100 rounded-2xl text-sm font-bold text-slate-700 outline-none focus:ring-4 focus:ring-indigo-500/5 focus:border-indigo-500 transition-all" placeholder="Masukkan password..." onChange={(e) => setPassword(e.target.value)} />
              </div>
            </div>

            <div className="flex items-center gap-2 px-1">
               <input type="checkbox" id="remember" checked={rememberMe} onChange={(e) => setRememberMe(e.target.checked)} className="w-4 h-4 rounded border-slate-300 text-indigo-600 focus:ring-indigo-500" />
               <label htmlFor="remember" className="text-xs font-bold text-slate-500">Ingat Saya</label>
            </div>

            <button disabled={loading} className="w-full py-5 bg-[#0F2650] text-white rounded-2xl font-black text-xs uppercase tracking-[0.3em] shadow-2xl shadow-indigo-900/20 hover:bg-[#1a3a70] transition-all flex items-center justify-center gap-3 mt-4">
