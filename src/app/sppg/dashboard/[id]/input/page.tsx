...
import {
  ArrowLeft, X, Utensils, Activity, RotateCcw, ArrowRight, CheckCircle2, Calendar, Layout, Loader2
} from 'lucide-react'
        ...
        <div className="bg-white rounded-[3.5rem] shadow-2xl shadow-indigo-900/10 overflow-hidden border border-white">
          ...
          <div className="p-8 lg:p-14 space-y-14">
            ...
            <div className="space-y-6">
              ...
              <div className={`border-4 border-dashed rounded-[3rem] p-16 flex flex-col items-center justify-center text-slate-400 relative group cursor-pointer transition-all shadow-inner ${isCompressing ? 'border-amber-300 bg-amber-50/20' : 'border-indigo-50 hover:border-indigo-500 hover:bg-indigo-50/20'}`}>

                {/* Preview or Upload Icon */}
                {previewUrl ? (
                  <div className="w-40 h-40 rounded-3xl overflow-hidden mb-6 shadow-xl border-2 border-white relative">
                ...
                <p className="text-[9px] font-bold text-slate-400 uppercase tracking-widest">Auto-Resize & Compress • Maks 1024px / 300KB / JPEG</p>
                ...
                {/* Compression Loading */}
                {isCompressing && (
                  <div className="mt-8 px-6 py-3 bg-amber-500 text-white rounded-[1.5rem] text-[10px] font-black flex items-center gap-3 animate-pulse shadow-xl shadow-amber-500/20 uppercase">
                    <Loader2 size={16} className="animate-spin" /> Mengompres foto...
                  </div>
                )}
                ...
                {!isCompressing && foto && (
                  <div className="mt-8 flex flex-col items-center gap-3">
                    <div className="px-6 py-3 bg-emerald-500 text-white rounded-[1.5rem] text-[10px] font-black flex items-center gap-3 animate-in zoom-in shadow-xl shadow-emerald-500/20 uppercase">
                      <CheckCircle2 size={16} /> {foto.name}
                    </div>
                    <span className="text-[9px] font-black text-emerald-600 uppercase tracking-widest">Ukuran Akhir: {compressedSize} KB</span>
                  </div>
                )}
            ...
            {/* ACTION BUTTON */}
            <button onClick={handleSimpan} disabled={loading || isCompressing} className="w-full py-8 bg-[#0F2650] text-white rounded-[2.5rem] font-black text-[12px] uppercase tracking-[0.5em] shadow-2xl shadow-indigo-900/30 hover:bg-[#6366F1] active:scale-[0.98] transition-all flex items-center justify-center gap-4 disabled:opacity-50 disabled:cursor-not-allowed">
              {loading ? 'SINKRONISASI DATA...' : isCompressing ? 'MENUNGGU KOMPRESI...' : <>{editId ? 'Simpan Perubahan Laporan' : 'Kirim Laporan Final'} <ArrowRight size={22} /></>}
            </button>
          </div>
        </div>
      </div>
